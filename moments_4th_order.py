"""4th-Order Moment Tensor Decomposition"""
...
def compute_4th_order_moments(X):
    """Compute 4th-order empirical moments tensor (symmetric)."""
    n, n_features = X.shape
    mu = np.mean(X, axis=0, keepdims=True)
    Xc = X - mu
    
    M = np.zeros((n_features, n_features, n_features, n_features))
    
    for i in range(n_features):
        for j in range(i, n_features):
            for k in range(j, n_features):
                for l in range(k, n_features):
                    moment = np.mean(Xc[:, i] * Xc[:, j] * Xc[:, k] * Xc[:, l])
                    for idx in set(permutations([i, j, k, l])):
                        M[idx] = moment
...
def decompose_tensor(M, rank=5):
    """Decompose symmetric 4th-order tensor using CP decomposition."""
    ...
    # Reconstruct
    recon_norm = tl.cp_to_tensor((factors.weights, [v_symmetric, v_symmetric, v_symmetric, v_symmetric]))
